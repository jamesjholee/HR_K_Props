...
ROOT = "https://api.propfinder.app"   # v1.4: /MLB/v2, /MLB/park-factors etc. live off root
...
def fetch(url, name, offline=False):
    ...
    data = r.json()
    h = _save(name, data)
    print(f"  [pull] {name}  hash={h}  {datetime.now(timezone.utc).isoformat(timespec='seconds')}")
    return data
...
def pitcher_splits(pid, season, offline=False):
    return fetch(f"{BASE}/pitcher-splits?pitcherId={pid}&season={season}",
                 f"splits_{pid}", offline)

def pitch_type_stats(pid, season, offline=False):
    return fetch(f"{BASE}/pitch-type-stats?playerId={pid}&season={season}"
                 f"&playerType=pitcher&handedness=all", f"pt_{pid}", offline)

def hr_matchup(pid, team_id, season, vs_rhb=None, vs_lhb=None,
               range_value=None, offline=False):
...
def fetch_post(url, body, name, offline=False):
    ...
    data = r.json()
    h = _save(name, data)
    print(f"  [post] {name}  hash={h}  {datetime.now(timezone.utc).isoformat(timespec='seconds')}")
    return data
...
def odds_v2(offline=False, books=("draftkings", "fanduel", "betmgm",
                                  "prizepicks", "underdog")):
...
def hit_data(pid, dates=None, offline=False, group="pitching"):
    ...
    return fetch_soft(f"{ROOT}/MLB/hit-data?playerId={pid}&group={group}",
                      f"hitdata_{pid}", offline)

def zone_matchups_all(season, offline=False):
    """Slate-wide zone matchups (verified live 2026-07-25, no pitcher path)."""
    return fetch_soft(f"{BASE}/zone-matchups?season={season}&minPitchUsage=0",
                      "zone_matchups_all", offline)
...
def mlb_gamelog(pid, season, offline=False):
    return fetch_soft(MLB_GAMELOG.format(pid=pid, season=season),
                      f"gamelog_{pid}", offline)
...
def park_factors(bat_side, offline=False):
    return fetch_soft(f"{ROOT}/MLB/park-factors?batSide={bat_side}",
                      f"park_factors_{bat_side}", offline)

def hr_risk(date_str, offline=False):
    """PropFinder's own model output — logged nightly as a benchmark column."""
    return fetch_soft(f"{BASE}/hr-risk?dates={date_str}",
                      f"hr_risk_{date_str}", offline)
...
def check_freshness(games, now_utc=None):
    ...
    for g in games:
        ...
        if not g.get("homePitcherId") or not g.get("visitorPitcherId"):
            alerts.append(f"STARTER MISSING: {label} — verify in MLB app (opener? scratch?)")
        if 0 < mins <= 90:
            if not g.get("homeBattingOrder"):
                alerts.append(f"LINEUP EMPTY T-{int(mins)}m: {label} home side — re-poll / check app")
            if not g.get("visitorBattingOrder"):
                alerts.append(f"LINEUP EMPTY T-{int(mins)}m: {label} visitor side — re-poll / check app")
    return alerts
...
def crosscheck_starters(games, mlb_sched):
    ...
    alerts = []
    if not mlb_sched:
        return ["MLB schedule unavailable — starter cross-check SKIPPED, verify manually"]
    probables = {}
    for d in mlb_sched.get("dates", []):
        for g in d.get("games", []):
            for side in ("home", "away"):
                ...
                if pid:
                    probables[t["team"]["id"]] = pid
    for g in games:
        for side, key in (("homeTeam", "homePitcherId"), ("visitorTeam", "visitorPitcherId")):
            tid = g.get(side, {}).get("id")
            pf = g.get(key)
            mlb = probables.get(tid)
            if pf and mlb and pf != mlb:
                alerts.append(f"STARTER MISMATCH {g[side].get('code')}: "
                              f"PropFinder={pf} vs MLB={mlb} — 5/27 RULE: verify before betting")
    return alerts
